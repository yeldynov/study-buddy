...
const Navbar = () => {
  ...
  return (
    <header className="w-full absolute z-10 bg-[#F0F0F0]">
      <nav className="max-w-[1440px] lg:hidden relative mx-auto flex justify-between items-center sm:px-16 px-6 py-4">
        <Link href="/">
          <Image
            src="/icon-phone.svg"
            alt="Car Hub Logo"
      ...
      {/* Desktop */}
      <nav className="max-w-[1440px] hidden lg:flex relative mx-auto justify-between items-center sm:px-16 px-6 py-4">
        <Link href="/" className="flex justify-center items-center">
          <Image
            src="/logo.svg"
            alt="Car Hub Logo"
        ...
        <ul className="list-none flex items-center justify-center gap-6 w-full">
          {navLinks.map((nav, index) => (
            <li
              key={nav.url}
              className={`font-bold cursor-pointer text-[16px] ${
